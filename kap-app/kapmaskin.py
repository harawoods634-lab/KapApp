...
with tab2:
    ...
    with col_a:
        ...
        order_m = st.number_input("Order (längd i meter)", min_value=1, value=1)
        raw_price_m3 = st.number_input("Råvarupris (kr/m³)", value=1.0, step=50.0)
        
        c1, c2 = st.columns(2)
        raw_t = c1.number_input("Råvara Tjocklek (mm)", value=0.0, step=0.1, key="rt_v33")
        raw_b = c2.number_input("Råvara Bredd (mm)", value=0.0, step=0.1, key="rb_v33")
        
        st.write("**Produkt (Nominella mått)**")
        nom_t = c1.number_input("Nominell Tjocklek (mm)", value=0.0, step=0.1, key="nt_v33")
        nom_b = c2.number_input("Nominell Bredd (mm)", value=0.0, step=0.1, key="nb_v33")
